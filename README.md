# Koah Skills

Repository of public agentic skills for working with Koah products

# Available Skills

1. [koah-integration](skills/koah-integration): Integrate Koah into your application, either as a publisher (AI app developer looking to monetize) or an advertiser looking to promote your brand on Koah.

# Get Started

## With [skills NPM package](https://www.npmjs.com/package/skills)

1. Install the skills NPM package if you haven't yet: `npm install -g skills`
2. Install the desired Koah skill, e.g. `npx skills add https://github.com/koahlabs/skills --skill koah-integration`. This installs the skill to your local project's `.agents/skills` folder.

## With Claude

1. Run `/plugins` within Claude to manage plugins
2. Add this repo (https://github.com/koahlabs/skills) under "Marketplaces".
3. Reload Claude's available skills list: `/reload-plugins`
3. Use your skill within Claude, e.g. `/koah-integration`


# LICENSE

MIT
